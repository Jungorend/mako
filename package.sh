...
case $1 in
    linux)
        do_build
        DEPLOY_GTK_VERSION=3 linuxdeploy --appimage-extract-and-run --executable=bin/mako \
                    --custom-apprun=package/AppRun \
                    --icon-file=package/icon.png \
                    --desktop-file=package/mako.desktop \
                    --plugin gtk \
                    --appdir=appimage $(find bin -name "lib*" -printf "-l%p ")
        cp bin/mako appimage/usr/bin
        cp -R Resources appimage/usr
        appimagetool --appimage-extract-and-run --comp zstd -g appimage "mako-${VERSION}.AppImage"
        ;;
    ...
    macos)
        ...
        xxd -ps -c1 bin/mako | tr -d '\n' | sed "s/$replace_fr/$replace_to/" | fold -w 2 | xxd -r -p > "$contents/MacOS/mako"
        chmod +x "$contents/MacOS/mako"

        hdiutil create -quiet -srcfolder "$bundle" out.dmg
        # NOTE: ULMO = lzma compression = Catalina+ only
        hdiutil convert -quiet out.dmg -format ULMO -o "mako-${VERSION}.dmg"
        rm out.dmg
        ;;

    *)
        echo "Unknown package flavor: $1"
        exit 1
        ;;
esac
